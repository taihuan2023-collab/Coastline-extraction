...
import torch
# from nets.unet_training import CE_Loss, Dice_loss, Focal_Loss, Sobel_edge_loss

from nets.unet_training import Hybrid_Adaptive_Loss
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #     loss      = loss + main_dice
            # loss = Sobel_edge_loss(outputs, pngs)
            loss = Hybrid_Adaptive_Loss(
                outputs,
                pngs,
                ...
                alpha=0.7,  # 区域损失权重稍大
                beta=0.3,  # 边缘约束稍弱
                dice_ratio=0.6,  # 60% Dice + 40% Focal
                use_ce=False
            )
