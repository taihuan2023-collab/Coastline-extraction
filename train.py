import datetime  # 用于获取时间戳，常用于保存日志、模型时命名；
import os  # 处理路径、环境变量等文件系统相关操作；
from functools import partial

import numpy as np
import torch
import torch.backends.cudnn as cudnn  # 调用 NVIDIA CUDNN 库做加速，后续可设置 cudnn.benchmark = True 加快训练；
import torch.distributed as dist
import torch.optim as optim  # 优化器：PyTorch 自带的优化器库，如 SGD、Adam 等；
from torch.utils.data import DataLoader  # 训练用的数据加载器

from nets.unet import Unet  # 导入模型结构
from nets.unet_training import get_lr_scheduler, set_optimizer_lr, \
    weights_init  # 训练策略相关函数：get_lr_scheduler：获取学习率调整策略set_optimizer_lr：设置优化器的当前学习率；weights_init：初始化模型权重（如 Xavier、He 正态分布等）
from utils.callbacks import EvalCallback, \
    LossHistory  # 训练回调函数EvalCallback：在训练过程中每轮或每 N 轮进行模型验证（保存最优权重）；LossHistory：记录训练和验证过程中的 loss 值，自动保存 loss 曲线图；
...
if __name__ == "__main__":
    ...
    pretrained = True  # 使用主干网络 ImageNet 权重
    model_path = ""  # 不加载整个模型权重
    ...
    #                   当使用SGD优化器时建议设置   Init_lr=1e-2           这里设置动态调整学习率就是学习步长
